...
import {createRoom,publicRoom,guessPoints,normalize,choices} from '../server/game.mjs';
test('word choice has three unique options',()=>{for(let i=0;i<100;i++)assert.equal(new Set(choices()).size,3);});
test('score rewards speed and clamps late guesses',()=>{assert.equal(guessPoints(60,60),500);assert.equal(guessPoints(30,60),300);assert.equal(guessPoints(-2,60),100);});
